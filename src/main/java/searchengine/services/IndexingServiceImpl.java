...
public class IndexingServiceImpl implements IndexingService {
    ...
    public IndexResponse search(String query, String site, String offset, String limit) {
        ...
        Map<Page, Map<Lemma, Float>> relevanceMap = new HashMap<>();
        for (Lemma lemma : lemmas) {
            List<Index> indexes = indexRepository.findByLemma(lemma.getId());
            for (Index index : indexes) {
                Page page = index.getPage();
                Map<Lemma, Float> rankMap;
                if (relevanceMap.containsKey(page)) {
                    rankMap = relevanceMap.get(page);
                } else {
                    rankMap = new HashMap<>();
                }
                if (rankMap.containsKey(lemma)) {
                    rankMap.put(lemma, rankMap.get(lemma) + index.getRank());
                } else {
                    rankMap.put(lemma, index.getRank());
                }
        ...
        Float max = absoluteRelevanceMap.values().stream().max(Float::compare).orElse(0f);
        if (max.equals(0)) {
            return new IndexErrorResponse(false, "Не найден максимум абсолютной релевантности");
        }
        ...
        List<Page> pages = relativeRelevanceMap.entrySet().stream().sorted(Comparator.comparing(Map.Entry<Page, Float>::getValue).reversed()).map(Map.Entry::getKey).collect(Collectors.toList());
        ...
        return new SearchResponse(true, searchItemList.size(), searchItemList);
    }
